...
namespace Pepper.Frontends.Maimai.Structures
{
    public class ScoreFormatter
    {
            ...
            (int, bool)? levelHints = null, bool? hasMultipleVersions = null)
        {
            ...
            {
                rating = MaimaiCommand.NormalizeRating(MaimaiCommand.GetFinalScore(record.Accuracy, chartConstant));
            }

            var comboText = MaimaiCommand.GetFormattedStatusString(record.FcStatus);
            var syncText = MaimaiCommand.GetFormattedStatusString(record.SyncStatus);
